...
from evaluation.metrics import (
    precision,
    precision_recall_min_denominator,
    recall,
    MAP,
    MAP_MIN_DEN,
    MRR,
...
def _create_empty_metrics_dict(
    cutoff_list,
    n_items,
    n_users,
    URM_train,
...
class Evaluator(object):
    ...
    def __init__(
        ...
        if not np.all(users_to_evaluate_mask):
            self._print(
                "Ignoring {} ({:4.1f}%) Users that have less than {} test interactions".format(
                    len(users_to_evaluate_mask) - np.sum(users_to_evaluate_mask),
                    100
                    * np.sum(np.logical_not(users_to_evaluate_mask))
                    / len(users_to_evaluate_mask),
    ...
    def evaluateRecommender(self, recommender_object):
            ...
            for cutoff in self.cutoff_list:
                ...
                if EvaluatorMetrics.F1.value in results_current_cutoff:
                    ...
                    if precision_ + recall_ != 0:
                        ...
                        results_current_cutoff[EvaluatorMetrics.F1.value] = (
        ...
        results_df = pd.DataFrame(
            columns=results_dict[self.cutoff_list[0]].keys(), index=self.cutoff_list
        )
    ...
    def get_user_relevant_items(self, user_id):
        ...
        assert (
            self.URM_test.getformat() == "csr"
        ), "Evaluator_Base_Class: URM_test is not CSR, this will cause errors in getting relevant items"
        return self.URM_test.indices[
            self.URM_test.indptr[user_id] : self.URM_test.indptr[user_id + 1]
        ]
    ...
    def get_user_test_ratings(self, user_id):
        ...
        return self.URM_test.data[
            self.URM_test.indptr[user_id] : self.URM_test.indptr[user_id + 1]
        ]

    def _compute_metrics_on_recommendation_list(
        self,
        ...
        assert len(recommended_items_batch_list) == len(
            ...
            self.EVALUATOR_NAME,
            len(recommended_items_batch_list),
        ...
        assert (
            scores_batch.shape[1] == self.n_items
        ), "{}: scores_batch contained scores for {} items, expected was {}".format(
        ...
        for batch_user_index in range(len(recommended_items_batch_list)):
            ...
            for cutoff in self.cutoff_list:
                ...
                results_current_cutoff[
                    EvaluatorMetrics.MAP_MIN_DEN.value
                ].add_recommendations(is_relevant_current_cutoff, relevant_items)
                results_current_cutoff[
                    EvaluatorMetrics.HIT_RATE.value
                ].add_recommendations(is_relevant_current_cutoff)

                results_current_cutoff[
                    EvaluatorMetrics.NOVELTY.value
                ].add_recommendations(recommended_items_current_cutoff)
                results_current_cutoff[
                    EvaluatorMetrics.AVERAGE_POPULARITY.value
                ].add_recommendations(recommended_items_current_cutoff)
                results_current_cutoff[
                ...
                ].add_recommendations(recommended_items_current_cutoff, test_user)
                results_current_cutoff[
                    EvaluatorMetrics.COVERAGE_USER_HIT.value
                ].add_recommendations(is_relevant_current_cutoff, test_user)
                results_current_cutoff[
                    EvaluatorMetrics.DIVERSITY_MEAN_INTER_LIST.value
                ].add_recommendations(recommended_items_current_cutoff)
                results_current_cutoff[
                ...
                results_current_cutoff[
                    EvaluatorMetrics.RATIO_AVERAGE_POPULARITY.value
                ].add_recommendations(recommended_items_current_cutoff)

                if (
                    EvaluatorMetrics.DIVERSITY_SIMILARITY.value
                    in results_current_cutoff
                ):
                    results_current_cutoff[
                        EvaluatorMetrics.DIVERSITY_SIMILARITY.value
                    ].add_recommendations(recommended_items_current_cutoff)

        if time.time() - self._start_time_print > 300 or self._n_users_evaluated == len(
            ...
            self._print(
                "Processed {} ({:4.1f}%) in {:.2f} {}. Users per second: {:.0f}".format(
