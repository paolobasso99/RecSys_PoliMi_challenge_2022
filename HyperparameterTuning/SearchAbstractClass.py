...
class SearchInputRecommenderArgs(object):
    def __init__(
        ...
        assert isinstance(
            CONSTRUCTOR_POSITIONAL_ARGS, list
        ), "CONSTRUCTOR_POSITIONAL_ARGS must be a list"
        assert isinstance(
            CONSTRUCTOR_KEYWORD_ARGS, dict
        ), "CONSTRUCTOR_KEYWORD_ARGS must be a dict"

        assert isinstance(
            FIT_POSITIONAL_ARGS, list
        ), "FIT_POSITIONAL_ARGS must be a list"
        assert isinstance(FIT_KEYWORD_ARGS, dict), "FIT_KEYWORD_ARGS must be a dict"

        assert isinstance(
            EARLYSTOPPING_KEYWORD_ARGS, dict
        ), "EARLYSTOPPING_KEYWORD_ARGS must be a dict"
...
class SearchAbstractClass(object):
    ...
    def _set_search_attributes(
        ...
        if save_model not in self._SAVE_MODEL_VALUES:
            raise ValueError(
                "{}: argument save_model must be in '{}', provided was '{}'.".format(
        ...
        if evaluate_on_test not in self._EVALUATE_ON_TEST_VALUES:
            raise ValueError(
                "{}: argument evaluate_on_test must be in '{}', provided was '{}'.".format(
        ...
        if save_model == "last" and recommender_input_args_last_test is None:
            self._write_log(
                "{}: argument save_model is 'last' but no recommender_input_args_last_test provided, saving best model on train data alone.".format(
                    self.ALGORITHM_NAME
                )
            )
            save_model = "best"
        ...
        self.evaluate_on_test = (
            "no" if self.evaluator_test is None else evaluate_on_test
        )
    ...
    def _init_metadata_dict(self, n_cases, hyperparameter_names):
        ...
        self.metadata_dict = {
            ...
            "hyperparameters_df": pd.DataFrame(
                columns=hyperparameter_names, index=np.arange(n_cases), dtype=object
            ),
            ...
            "result_on_earlystopping_df": pd.DataFrame(dtype=object)
            if issubclass(self.recommender_class, Incremental_Training_Early_Stopping)
            else None,
            "time_df": pd.DataFrame(
                columns=["train", "validation", "test"], index=np.arange(n_cases)
            ),
    ...
    def _evaluate_on_validation(
        ...
        if not was_already_evaluated_flag:
            ...
            for key in current_fit_hyperparameters.keys():
                self.metadata_dict["hyperparameters_df"].loc[
                    self.model_counter, key
                ] = current_fit_hyperparameters[key]
            ...
            if isinstance(recommender_instance, Incremental_Training_Early_Stopping):
                for (
                    ...
                    recommender_instance.get_early_stopping_final_epochs_dict().items()
                ):
                    self.metadata_dict["hyperparameters_df"].loc[
                        self.model_counter, epoch_key
                    ] = int(epoch_value)

                ## This is to ensure backward compatibility
                if "result_on_earlystopping_df" not in self.metadata_dict:
                    self.metadata_dict["result_on_earlystopping_df"] = (
                        pd.DataFrame(dtype=object)
                        if issubclass(
                            self.recommender_class, Incremental_Training_Early_Stopping
                        )
                        else None
                    )
            ...
            self.metadata_dict["hyperparameters_df"].loc[self.model_counter] = (
                ...
                .loc[was_already_evaluated_index]
                .copy()
            )
            result_df = (
                self.metadata_dict["result_on_validation_df"]
                .loc[was_already_evaluated_index]
                .copy()
            )
        ...
        if self.metadata_dict["result_on_validation_df"] is None:
            ...
            ] = create_result_multiindex_dataframe(self.n_cases, result_df)
        ...
        self.metadata_dict["time_df"].loc[
            self.model_counter, "validation"
        ] = evaluation_time

        self.metadata_dict["time_on_train_avg"] = self.metadata_dict["time_df"][
            "train"
        ].mean(axis=0, skipna=True)
        self.metadata_dict["time_on_train_total"] = self.metadata_dict["time_df"][
            "train"
        ].sum(axis=0, skipna=True)
        self.metadata_dict["time_on_validation_avg"] = self.metadata_dict["time_df"][
            "validation"
        ].mean(axis=0, skipna=True)
        self.metadata_dict["time_on_validation_total"] = self.metadata_dict["time_df"][
            "validation"
        ].sum(axis=0, skipna=True)
    ...
    def _evaluate_on_test(
        ...
        if was_already_evaluated_flag:
            result_df_test = (
                self.metadata_dict["result_on_test_df"]
                .loc[was_already_evaluated_index]
                .copy()
            )
        ...
        if print_log:
            self._write_log(
                "{}: Config evaluated with evaluator_test. Config: {} - results:\n{}\n".format(
                    self.ALGORITHM_NAME, current_fit_hyperparameters_dict, result_string
                )
            )
        ...
        if self.metadata_dict["result_on_test_df"] is None:
            ...
            self.metadata_dict[
                "result_on_test_df"
            ] = create_result_multiindex_dataframe(self.n_cases, result_df_test)
        ...
        self.metadata_dict["time_df"].loc[
            self.model_counter, "test"
        ] = evaluation_test_time
        self.metadata_dict["time_on_test_avg"] = self.metadata_dict["time_df"][
            "test"
        ].mean(axis=0, skipna=True)
        self.metadata_dict["time_on_test_total"] = self.metadata_dict["time_df"][
            "test"
        ].sum(axis=0, skipna=True)
    ...
    def _evaluate_on_test_with_data_last(self):
        ...
        if self.resume_from_saved and self.metadata_dict["result_on_last"] is not None:
            self._print(
                "{}: Resuming '{}'... Result on last already available.".format(
        ...
        self._print(
            "{}: Evaluation with constructor data for final test. Using best config: {}".format(
        ...
        assert (
            self.metadata_dict["hyperparameters_best"] is not None
        ), "{}: Best hyperparameters not available, the search might have failed.".format(
            self.ALGORITHM_NAME
        )
    ...
    def _objective_function(self, current_fit_hyperparameters_dict):
        ...
        try:
            ...
            (
                was_already_evaluated_flag,
                was_already_evaluated_index,
            ) = self._was_already_evaluated_check(current_fit_hyperparameters_dict)
            ...
            if self.metadata_dict["result_on_validation_best"] is None:
                ...
                new_best_config_found = (
                    best_solution_val < result_series[self.metric_to_optimize]
                )
            ...
            if new_best_config_found:
                ...
                self._write_log(
                    "{}: Config {} {}. Config: {} - results: {}\n".format(
                        ...
                        "is suboptimal"
                        if not was_already_evaluated_flag
                        else "was already explored at index {}".format(
                            was_already_evaluated_index
                        ),
                        current_fit_hyperparameters_dict,
                        get_result_string_prettyprint(result_series, n_decimals=7),
            ...
            if new_best_config_found:
                ...
                ] = current_fit_hyperparameters_dict.copy()
        ...
        except MEMORY_ERROR_EXCEPTION_TUPLE if self.terminate_on_memory_error else (
            NeverMatch
        ) as e:
            self._print(
