...
from Recommenders.FeatureWeighting.Cython.CFW_DVV_Similarity_Cython import (
    CFW_DVV_Similarity_Cython,
)
...
def run_KNNRecommender_on_similarity_type(
    ...
    if not is_set_similarity:
        ...
        if allow_bias_ICM:
            hyperparameters_range_dictionary["ICM_bias"] = Real(
                low=1e-2, high=1e3, prior="log-uniform"
            )
    ...
    local_hyperparameter_search_space = {
        **hyperparameters_range_dictionary,
        **original_hyperparameter_search_space,
    }
...
def runHyperparameterSearch_Collaborative(
    ...
    try:
        ...
        if (
            recommender_class is SLIMElasticNetRecommender
            or recommender_class is MultiThreadSLIM_SLIMElasticNetRecommender
        ):
