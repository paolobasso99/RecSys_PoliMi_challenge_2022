...
def _extend_dataframe(initial_dataframe, new_rows):
    ...
    if isinstance(initial_dataframe.index, pd.MultiIndex):
        ...
        cases_cutoff_multiindex = pd.MultiIndex.from_product(
            [
                range(
                    first_level_index.max() + 1, first_level_index.max() + 1 + new_rows
                ),
                second_level_index,
            ]
        )
        ...
        new_df = pd.DataFrame(
            columns=initial_dataframe.columns,
            index=range(len(initial_dataframe), len(initial_dataframe) + new_rows),
        )
...
class TimeoutError(Exception):
    def __init__(self, max_total_time_seconds, current_total_time):
        (
            max_total_time_seconds_value,
            max_total_time_seconds_unit,
        ) = seconds_to_biggest_unit(max_total_time_seconds)
        (
            current_total_time_seconds_value,
            current_total_time_seconds_unit,
        ) = seconds_to_biggest_unit(current_total_time)
...
class SearchBayesianSkopt(SearchAbstractClass):
    ...
    def __init__(
        ...
        assert (
            evaluator_validation is not None
        ), "{}: evaluator_validation must be provided".format(self.ALGORITHM_NAME)
    ...
    def _resume_from_saved(self):
        ...
        try:
            ...
            if n_cases_in_loaded_data < self.n_cases:
                ...
                self._write_log(
                    "{}: Extending previous number of cases from {} to {}.\n".format(
            ...
            self._write_log(
                "{}: Resuming '{}' Failed, no such file exists.\n".format(
            ...
            self._write_log(
                "{}: Resuming '{}' Failed, generic exception: {}.\n".format(
        ...
        hyperparameters_df = self.metadata_dict["hyperparameters_df"][
        ...
        assert (
            hyperparameters_df[: self.model_counter].notna().any(axis=1).all()
        ...
        for hyperparameter_index, hyperparameter_name in enumerate(
            self.hyperparams_names
        ):
            if (
                isinstance(self.hyperparams_values[hyperparameter_index], Categorical)
                and len(self.hyperparams_values[hyperparameter_index].categories) == 1
            ):
                hyperparameters_df[hyperparameter_name] = self.hyperparams_values[
        ...
        for index in range(self.model_counter):
            ...
            is_validation_valid = (
                result_on_validation_df is not None
                and result_on_validation_df[self.metric_to_optimize]
                .notna()[index]
                .any()
            )
            assert (
                is_exception == is_validation_valid
        ...
        if result_on_validation_df is not None:
            ...
            result_on_validation_df = result_on_validation_df.loc[
                : self.model_counter - 1
            ]
            result_on_validation_list_input = (
                -result_on_validation_df[self.metric_to_optimize].loc[
        ...
        assert len(hyperparameters_list_input) == len(
            result_on_validation_list_input
        ...
        self._print(
            "{}: Resuming '{}'... Loaded {} configurations.".format(
    ...
    def search(
        ...
        if self.max_total_time is not None:
            ...
            self._print(
                "{}: The search has a maximum allotted time of {:.2f} {}".format(
        ...
        try:
            if self.resume_from_saved:
                (
                    ...
                    result_on_validation_list_saved,
    ...
    def _objective_function_list_input(
        self, current_fit_hyperparameters_list_of_values
    ):
        """
        ...
        total_current_time = (
            self.metadata_dict["time_on_train_total"]
            + self.metadata_dict["time_on_validation_total"]
        )
        estimated_last_time = (
            self.metadata_dict["time_df"]
            .loc[self.metadata_dict["hyperparameters_best_index"]][
                ["train", "validation"]
            ]
            .sum()
            if self.metadata_dict["hyperparameters_best_index"] is not None
            else 0
        )
        ...
        if self.max_total_time is not None:
            ...
            if (
                self.recommender_input_args_last_test is None
                and total_current_time > self.max_total_time
            ):
                raise TimeoutError(self.max_total_time, total_current_time)
            elif (
                self.recommender_input_args_last_test is not None
                and total_current_time + estimated_last_time > self.max_total_time
            ):
                raise TimeoutError(
                    self.max_total_time, total_current_time + estimated_last_time
                )
        ...
        # The search can only progress if there is at least a valid config in the initial random start
        if (
            self.metadata_dict["result_on_validation_df"] is None
            and self.model_counter >= self.n_random_starts
        ):
            raise NoValidConfigError()
