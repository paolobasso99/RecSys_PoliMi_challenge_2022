...
class SearchSingleCase(SearchAbstractClass):
    ...
    def __init__(
        ...
        super(SearchSingleCase, self).__init__(
            recommender_class,
    ...
    def _resume_from_saved(self):
        ...
        assert (
            len(self.metadata_dict["hyperparameters_df"]) == 1
        ), "{}: Loaded data inconsistent with current search algorithm".format(
            self.ALGORITHM_NAME
        )
    ...
    def _evaluate_on_validation(
        ...
        if self.evaluator_validation is not None:
            return super(SearchSingleCase, self)._evaluate_on_validation(
                current_fit_hyperparameters,
                was_already_evaluated_flag,
                was_already_evaluated_index,
            )
            ...
            return (
                {self.metadata_dict["metric_to_optimize"]: 0.0},
                "",
    ...
    def search(
        ...
        assert (
            fit_hyperparameters_values is not None
        ), "{}: fit_hyperparameters_values must contain a dictionary".format(
            self.ALGORITHM_NAME
        )
