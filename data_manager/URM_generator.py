...
class URMGenerator(object):
    def __init__(
        ...
        users_id = np.unique(
            np.concatenate(
                (self.dataset_training["user_id"], self.dataset_val["user_id"])
            )
        )
        items_id = np.unique(
            np.concatenate(
                (self.dataset_training["item_id"], self.dataset_val["item_id"])
            )
        )
    ...
    def generate_explicit_URM(
        ...
        URM_train = self._generate_explicit_split_URM(
            self.dataset_training, log_base, views_weight, details_weight
        )
        URM_val = self._generate_explicit_split_URM(
            self.dataset_val, log_base, views_weight, details_weight
        )
